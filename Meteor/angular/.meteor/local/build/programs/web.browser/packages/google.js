...
Google.requestCredential = function (options, credentialRequestCompleteCallback) {           // 8
  ...
  if (options.prompt != null) {                                                              // 51
    ...
    loginUrlParameters.prompt = 'consent'                                                    // 54
                                                                                             ...
                                                                                             // 56
  var loginStyle = OAuth._loginStyle('google', config, options);                             // 57
  // https://developers.google.com/accounts/docs/OAuth2WebServer#formingtheurl               // 58
  _.extend(loginUrlParameters, {                                                             // 59
    "response_type": "code",                                                                 // 60
    "client_id":  config.clientId,                                                           // 61
    "scope": scope.join(' '), // space delimited                                             // 62
    "redirect_uri": OAuth._redirectUri('google', config),                                    // 63
    "state": OAuth._stateParam(loginStyle, credentialToken, options.redirectUrl)             // 64
  });                                                                                        // 65
  var loginUrl = 'https://accounts.google.com/o/oauth2/auth?' +                              // 66
    _.map(loginUrlParameters, function(value, param){                                        // 67
      return encodeURIComponent(param) + '=' + encodeURIComponent(value);                    // 68
    }).join("&");                                                                            // 69
                                                                                             // 70
  OAuth.launchLogin({                                                                        // 71
    loginService: "google",                                                                  // 72
    loginStyle: loginStyle,                                                                  // 73
    loginUrl: loginUrl,                                                                      // 74
    credentialRequestCompleteCallback: credentialRequestCompleteCallback,                    // 75
    credentialToken: credentialToken,                                                        // 76
    popupOptions: { height: 600 }                                                            // 77
  });                                                                                        // 78
};                                                                                           // 79
                                                                                             // 80
///////////////////////////////////////////////////////////////////////////////////////////////
